...
function UserProvider({ children }: { children: React.ReactNode }) {
  ...
  const initialArgs: UserState =
    !!party && !!token ? { isAuthenticated: true, token, party } : { isAuthenticated: false };
...
function loginDamlHubUser(): void {
  window.location.assign(`https://${damlHubLoginUrl}`);
}

function signOut(event: React.FormEvent, dispatch: React.Dispatch<LoginAction>, history: History): void {
...
export { UserProvider, useUserState, useUserDispatch, loginUser, loginDamlHubUser, signOut };
