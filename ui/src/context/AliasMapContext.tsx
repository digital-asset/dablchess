...
import { Aliases } from '@daml-ts/chess-0.5.0/lib/Alias';
...
class AliasMap {
  constructor(
    public aliasToParty: Record<string, string>,
...
function AliasMapProvider({ children }: { children: React.ReactNode }) {
  ...
  return <AliasMapContext.Provider value={aliasMap}>{children}</AliasMapContext.Provider>;
}
