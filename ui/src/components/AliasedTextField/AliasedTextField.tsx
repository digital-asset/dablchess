...
// An autocomplete text field to set values based on aliases.
...
// A text field that draws input from known aliases.
export default function AliasedTextField({onChange, placeholder} : AliasedTextfieldProps){
  const aliases = useAliases();
  const [value, setValue] = useState<string|null>(null);
  function handleChange(event: any, value:(AutocompleteState | null), reason: any){
    if(value === null){
      return;
    } else {
